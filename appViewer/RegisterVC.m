    ...
    if([self.password.text isEqualToString:@""]||[self.username.text isEqualToString:@""]){
        ...
        [HttpHelper get:url params:dic success:^(id respond){
            ...
            if([B isEqualToString:@"-1"]){
                UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"我的警告框" message:@"该账号已经被登陆" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
                [alert show];
            }else if([B isEqualToString:@"1"]){
                UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"我的警告框" message:@"注册成功" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
                [alert show];
            }
        } failure:^(NSError * error){
            NSLog(@"%@",error);
            UIAlertView * alert = [[UIAlertView alloc]initWithTitle:@"我的警告框" message:@"注册失败 " delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
            [alert show];
        }];
    }
}
